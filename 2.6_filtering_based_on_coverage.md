...
Turns out neither of these was as good as GATK, so I used that instead.  I also wrote an R script to identify all sites whose rolling average of coverage within a sample was greater than 4* the standard deviation plus the mean coverage of that sample.  This was based on inspection of the samples at a range of coverages.  The average cutoff was 2.09 times the mean coverage, which is perfect because it should effectively exclude many of the duplicated regions (which should have 2X or higher converage).  For chr02a, out of 1793350 polymorphic sites, 10060 exceeded the coverage cutoff in at least one sample.
...
```bash
vcftools --gzvcf FandM_chrX_BSQR_jointgeno_allsites_withpapio_filtered1_SNPsonly.vcf.gz --out FandM_chrX_BSQR_jointgeno_allsites_withpapio_filtered2_coverage_SNPsonly.vcf.gz --exclude-positions chrX_positions_to_exclude.txt
```
