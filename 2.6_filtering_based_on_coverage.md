...
Turns out neither of these was as good as GATK, so I used that instead.  I also wrote an R script to identify all sites whose rolling average of coverage within a sample was greater than 4* the standard deviation plus the mean coverage of that sample.  This was based on inspection of the sample with the lowest coverage and would be very permissive for samples with higher coverage (and thus lower relative sd).
